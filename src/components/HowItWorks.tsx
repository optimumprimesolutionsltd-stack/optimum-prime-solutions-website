...
const steps = [
  ...
  { icon: HeadphonesIcon, num: '04', title: 'Ongoing Support', desc: 'Business-hours remote support with < 1 hour response time. Regular updates, health checks, and on-site visits when you need them.', color: 'from-amber-500 to-orange-400' },
];

export default function HowItWorks() {
  return (
    <section className="relative py-24 overflow-hidden bg-slate-800 text-white">
      <div className="absolute left-0 top-0 h-72 w-72 rounded-full bg-red-100/40 blur-3xl" />
      <div className="absolute right-0 bottom-10 h-72 w-72 rounded-full bg-slate-800/20 blur-3xl" />
            ...
            {steps.map((step, i) => {
              ...
              return (
                <motion.div key={step.num} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                  ...
                  className="relative text-center group overflow-hidden rounded-[28px] border border-white/10 bg-slate-800/95 p-8 shadow-2xl shadow-slate-200 transition duration-300 hover:-translate-y-1 hover:shadow-cyan-200/30">
                  <div className="absolute inset-x-0 top-0 h-1 bg-red-600" />
                  <div className="relative mx-auto mb-6 mt-6 h-24 w-24 rounded-[28px] bg-slate-900 p-1 shadow-xl">
                    <div className="h-full w-full rounded-[26px] bg-slate-800 flex items-center justify-center">
                  ...
                  <p className="mt-4 text-sm text-slate-400 leading-relaxed">{step.desc}</p>
                </motion.div>
              );
            })}
          </div>
...
Join 2.5 million+ businesses worldwide already running on TallyPrime. Get a free, personalized demo from Kenya’s certified TallyPrime partner — and be live in 5 business days.
            </p>
