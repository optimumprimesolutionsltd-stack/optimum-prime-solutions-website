...
// w/h are the assets' real intrinsic sizes. Every one of these is a wordmark
// wider than it is tall, so the node below is a plate, not a circle — a circle
// crops to the short side and leaves the logo unreadable.
const partners = [
  { name: 'TallyPrime',  logo: '/tally-solutions-new-logo.png', angle: 0,   color: '#dc2626', w: 548,  h: 238 },
  { name: 'EOS®',        logo: '/partner-eos.png',        angle: 60,  color: '#0ea5e9', w: 800,  h: 400 },
  { name: 'Biz Analyst', logo: '/partner-biz-analyst.png', angle: 120, color: '#34d399', w: 512,  h: 512 },
  { name: 'TSplus',      logo: '/partner-tsplus.png',     angle: 240, color: '#8b5cf6', w: 300,  h: 300 },
  { name: 'KRA eTIMS',   logo: '/kra-logo-official.png', angle: 300, color: '#16a34a', w: 1858, h: 524 },
];
...
function OrbitIcon({ partner, radius, duration, delay }: {
  ...
  return (
    ...
    >
      ...
      >
        {/* White plate rather than a tinted circle: three of these logos ship on
          ...
          >
            <div className="relative scale-75 sm:scale-100" style={{ width: 400, height: 400 }}>
              ...
              {/* Orbiting partner icons. One shared duration, so the ring keeps
                  ...
                  are now wide enough that drifting means colliding. 60deg apart
                  puts 180px between plate centres, which two 112px plates clear. */}
              {partners.map((p) => (
                <OrbitIcon
                  key={p.name}
                  partner={p}
