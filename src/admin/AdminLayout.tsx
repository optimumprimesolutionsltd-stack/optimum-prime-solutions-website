...
import FeedbackManager from './editors/FeedbackManager';
...
const tabs: { id: TabId; label: string; icon: typeof LayoutDashboard }[] = [
  ...
  // In-app feedback, posted directly by each product's own server.
...
export default function AdminLayout({ onLogout, isFullAdmin }: Props) {
  ...
  const baseAccessibleTabs = isFullAdmin ? new Set(tabs.map(t => t.id)) : new Set(['dashboard', 'leads', 'wip', 'customers', 'renewals', 'subscriptions', 'feedback', 'workshop', 'webinar']);
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="h-16 flex items-center justify-between bg-white border-b border-slate-200 px-4 lg:px-8 shrink-0 shadow-sm">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <button onClick={() => setSidebar(true)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 active:bg-slate-200 lg:hidden transition-colors shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {newLeads > 0 && (
              <button onClick={() => setTab('leads')} className="flex items-center gap-1.5 rounded-lg bg-red-50 px-3 sm:px-4 py-2 text-xs font-semibold text-red-700 hover:bg-red-100 transition-colors active:bg-red-200">
                <span className="h-2 w-2 rounded-full bg-red-600 animate-pulse" />
                <span className="hidden sm:inline">{newLeads} new lead{newLeads > 1 ? 's' : ''}</span>
