    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <div className="space-y-4">
          <div><label className="block text-xs font-semibold text-slate-600 mb-1.5">Address / Location</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs font-semibold text-slate-600 mb-1.5 flex items-center gap-1"><MessageCircle className="h-3 w-3" />WhatsApp Number</label>
              ...
              <p className="text-[10px] text-slate-400 mt-1">Numbers only, no + or spaces</p></div>
            <div><label className="block text-xs font-semibold text-slate-600 mb-1.5">Google Maps Embed URL</label>
        ...
        <div key={key} className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2.5">
              <label className="block text-xs font-semibold text-slate-600">Edit {label}</label>
              {c[key].map((val: string, i: number) => (
                <div key={i} className="flex gap-2">
                  <input value={val} onChange={e => updList(key, i, e.target.value)} placeholder={placeholder}
                    className="flex-1 rounded-lg border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-accent" />
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 space-y-2">
                {c[key].filter((v: string) => v).length > 0 ? (
                  c[key].filter((v: string) => v).map((val: string, i: number) => (
                    <div key={i} className="text-sm text-slate-800 flex items-center gap-2">
