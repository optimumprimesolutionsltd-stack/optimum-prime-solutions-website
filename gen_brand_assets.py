...
def artwork():
    ...
    Returns `lockup(ink, accent)`, `mark(ink, accent)` and `compact(ink,
    accent)`, each of which paints the masks and trims to the artwork. Passing the same colour for both gives
    ...
    def compact(ink, accent=RED):
        """The lockup without the tagline, for the site header.

        At the 40px the header gives it, "PRIME SOLUTIONS" sets at about four
        pixels tall and reads as a grey smudge under the wordmark. Dropping it
        lets the rest of the lockup fill that height instead.
        """
        return _trim(_paint([(cloud, ink), (wordmark, ink),
                             (arrow, accent), (tittle, accent)]))

    return lockup, mark, compact


def main():
    lockup, mark, compact = artwork()
    ...
    save("optimum-logo-header.png", _fit(compact(BLACK), 640))
    save("optimum-logo-header-knockout.png", _fit(compact(WHITE), 640))
